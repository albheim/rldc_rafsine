...
tune.register_env("DCEnv", lambda config: DCEnv())
tune.register_env("DCEnvCont", DCEnv)
tune.register_env("DCEnvDisc", DCEnv)
...
analysis = tune.run(
    ...
    local_dir=os.path.join("results", "DCEnv", "RAFSINE" if args.rafsine else "SIMPLE", "SAC", args.model),
